...
def bipartite_graph_coloring(part_x: list, part_y: list, edges: list) -> list:
  ...
  part_x = [{"vertex": v, "dummy": False} for v in part_x]
  part_y = [{"vertex": v, "dummy": False} for v in part_y]
  ...
  for i in range(abs(len(part_x) - len(part_y))):
    if len(part_x) < len(part_y):
      part_x.append({"vertex": f"dummy_{i}", "dummy": True})
    else:
      part_y.append({"vertex": f"dummy_{i}", "dummy": True})
  
  # step2: Make the graph k-regular by adding some edges
  max_degree_of_part_x = max([len([edge for edge in edges if edge["v_x"] == vertex["vertex"]]) for vertex in part_x])
  max_degree_of_part_y = max([len([edge for edge in edges if edge["v_y"] == vertex["vertex"]]) for vertex in part_y])
  max_degree = max(max_degree_of_part_x, max_degree_of_part_y)
  degree_of_part_x = {vertex["vertex"]: len([edge for edge in edges if edge["v_x"] == vertex["vertex"]]) for vertex in part_x}
  degree_of_part_y = {vertex["vertex"]: len([edge for edge in edges if edge["v_y"] == vertex["vertex"]]) for vertex in part_y}
  ...
  while len(list_of_edges_to_be_divided) > 0:
    ...
    if max_degree % 2 == 0:
      ...
      if max_degree == 2:
        matchings.append(return_edges[0])
        matchings.append(return_edges[1])
      else:
        list_of_edges_to_be_divided.append(return_edges[0])
        list_of_edges_to_be_divided.append(return_edges[1])
    else:
      t = int(math.log2(max_degree * len(part_x))) + 1
      edges_to_be_divided *= (2 ** t) // max_degree
      edges_to_be_divided += [{"v_x": part_x[i], "v_y": part_y[i], "dummy": True} for i in range(len(part_x)) for j in range((2 ** t) % max_degree)]
      matching = edges_to_be_divided
      while len(matching) > len(part_x):
        return_edges = divide_graph(matching)
        numbers_of_dummy = [len([edge for edge in return_edges[0] if edge["dummy"]]) for i in range(2)]
